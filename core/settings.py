...
# O ambiente local substitui DATABASES e não interpreta conexões externas.
IS_LOCAL_PROFILE = os.environ.get('DJANGO_SETTINGS_MODULE', '') in {'core.settings_local', 'core.settings_test'}
...
if IS_LOCAL_PROFILE:
    ...
    local_database = urlparse(DATABASE_URL).hostname in {'localhost', '127.0.0.1'}
    DATABASES = {'default': dj_database_url.parse(DATABASE_URL, conn_max_age=600, ssl_require=not local_database)}
else:
    DATABASES = {'default': {
...
STATICFILES_DIRS = [BASE_DIR / 'sistema_isp' / 'static']
